...
export const VoteSystem = defineComponent({
  props: {
    currentLocale: {
      type: String,
      default: 'es',
  ...
  setup(props, { slots }) {
    const i18n = getI18N({ currentLocale: props.currentLocale })
    ...
    const { categoryName = '', candidates } = pageInfo.value ?? {}
    ...
    const removeMinHeigh = () => {
      if (containerRef.value) containerRef.value.style.minHeight = ''
    }

    const addMinheight = () => {
      if (containerRef.value)
        containerRef.value.style.minHeight = `${containerRef.value.offsetHeight}px`
    }
    ...
    if (categoryCode.value === MAX_CATEGORIES) {
      return (
        <VoteFinal
          candidates={candidatesByCategory}
          votes={votes.value}
          setCategory={(payload: number) => (categoryCode.value = payload)}
    ...
    return () => (
      <>
        <CategoryTitle isChanging={isChanging.value}>{categoryName}</CategoryTitle>
        ...
        <footer class="flex flex-col sm:flex-row justify-between px-4 items-center gap-x-20 rounded bg-black/50 backdrop-blur-xl mt-10 py-2">
          ...
          <div class="flex justify-center items-center gap-x-4 px-2 rounded py-2">
            ...
            <span class="text-lg font-semibold">
              {i18n.CATEGORY}{' '}
              <span class="text-3xl">
                {categoryCode.value + 1}/{MAX_CATEGORIES}
              </span>
            </span>
